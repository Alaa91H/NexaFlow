...
## Unreleased — external ingress security

- External task links now require explicit enablement, a per-task token, and confirmation before execution. Details offers enable, rotate, revoke and share controls. Invalid or tokenless links open task review.
- Installation-local deep-link and webhook capabilities are omitted from portable backups. Imported tasks remain disabled, and webhook tokens must be regenerated after review.
- Loopback webhooks require a token, canonicalize paths, bound request/header reads before allocation, and enforce a total read deadline, handler cap and global request limit. Rejection counters distinguish authentication, size, timeout and rate limits.
- HTTP actions are HTTPS-only. Local/private destinations require explicit opt-in and the Android 17 runtime permission. Every DNS address is checked and pinned into the transport; redirects are revalidated and cleartext downgrades rejected.
- HTTP output includes truncation/content-type/final-URL/byte-count metadata; oversized bodies fail without exposing partial JSON.
- Imports have stream-byte, JSON-depth/token and model quotas. Room schema 19 adds nullable local capabilities. Elevated failure logs omit configuration and dynamic messages.
- Release CI audits the merged manifest against exact platform permission gates. Widget receivers are private, and the backup save target accepts content streams rather than arbitrary filesystem paths.
