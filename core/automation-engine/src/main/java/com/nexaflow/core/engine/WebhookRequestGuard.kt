...
object WebhookRequestGuard {
    ...
    fun readRequest(input: InputStream): Request {
        ...
        val queryTokens = uri.rawQuery.orEmpty().split('&').filter {
            URLDecoder.decode(it.substringBefore('='), "UTF-8") == "token"
        }
        ...
        while (true) {
            ...
            require(':' in line && line.substringBefore(':').matches(Regex("[!#$%&'*+.^_`|~0-9A-Za-z-]+")))
    ...
    fun canonicalPath(raw: String): String {
        ...
        require(!decoded.startsWith("//") && decoded.none { it.isISOControl() || it == '\\' || it == '%' || it == '?' || it == '#' })
