...
| فتح تحديثات Play أو متجر التطبيقات | إجراءات موجودة | فتح صفحة/تطبيق المتجر فقط، **ليس** تحديثاً تلقائياً لتطبيقات الغير |
...
| 10 | فتح متجر التطبيقات | أجهزة مزوّدين | موجود؛ يفتح التطبيق إن وُجد وإلا fallback |
...
|---|---|---|
| تحديث تطبيقات Google Play أو متجر التطبيقات تلقائياً | Play Core يحد in-app update بالتطبيق المستدعي؛ ملكية الحزمة/المثبت وموافقة المستخدم تقيد التحديثات الخارجية [1] [8] | فتح إدارة تحديثات المتجر، أو تفعيل auto-update في إعدادات المتجر، أو MDM/store owner خارج التطبيق |
