...
The builder combines the device profile and capability snapshot. Backend-specific Root and Shizuku commands require the matching provider; Shizuku also requires a connected UserService. Compatibility is recalculated when the builder resumes after permission settings. Unsupported functions are hidden. Capability-mapped functions can show locked rows, but some legacy permission-gated functions are still filtered out before that classification; consistent permission-required discovery remains open. Catalog visibility is not proof that a particular OEM operation will succeed at runtime.
