...
| **P5 — Notifications & widget** ✅ | System-surface notifications with NexaFlow brand tint limited to icons/actions (`setColorized(false)`), so SystemUI owns text/background contrast across light, dark, dynamic-color and OEM themes; widget = QS tiles: card radius 24dp via NexaFlowCard → shapes.medium, tile icon/label synced to the bound task at runtime | `MonitoringService.kt`, `OemAutostartNotifier.kt`, `ReminderAlarmReceiver.kt`, `SystemController.kt` | contrast regression tests + module tests |
| **P6 — Picker sheets (this pass)** ✅ | Selection tasks open as full-height `ModalBottomSheet` instead of centered dialogs (Google pattern): action picker, exit-action picker, plugin picker, app picker, Bluetooth device picker, calendar picker, constraint-type picker, ROM live-key picker, notification-buttons picker, tile-binding dialog, Evolution-X settings browser. Kept as real dialogs per M3: text-input editors (reply variable, ROM value), time picker, About info, location-interval settings form (already Google checkmark-rows) | `ActionPickerDialog.kt`, `ExitActionPickerDialog.kt`, `PluginPickerDialog.kt`, `AppPickerDialog.kt`, `BluetoothDevicePickerDialog.kt`, `CalendarPickerDialog.kt`, `ConstraintEditorCard.kt`, `TriggerEditorCard.kt`, `ActionConfigEditor.kt`, `WidgetsScreen.kt`, `CapabilityCenterScreen.kt` | assembleDebug green; affected-module tests + detekt green |
