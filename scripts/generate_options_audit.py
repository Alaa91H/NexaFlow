...
def main():
    ...
    ap(
        ...
        "Static extraction of enum/editor branches and directly referenced config keys. "
        "Delegated/shared/dynamic configuration may not be captured. This is not runtime proof or an exhaustive option contract; "
        "see CONFIGURATION.md and CAPABILITY_CATALOG.md for current guides."
    )
    ap("")
    if missing:
        ...
        ap(f"Editor localization: **{total_keys} keys present in all 11 resource sets; English fallback may remain**.")
    ap("")
    ...
    print(
        ...
        f"{'key parity' if not missing else 'MISSING ' + str(sorted(missing))}"
    )
