...
fun VariablesScreen(navController: NavController) {
    ...
    val variables = viewModel.variables.collectAsLazyPagingItems()
    ...
    Scaffold(
        ...
        LazyColumn(
            ...
            if (variables.itemCount == 0) {
                when (val refresh = variables.loadState.refresh) {
                    ...
                    is LoadState.Error -> item {
                        Column(
                            ...
                            Text(
                                text = stringResource(R.string.variables_load_error),
                                style = MaterialTheme.typography.bodyMedium,
                                color = MaterialTheme.colorScheme.secondary
                            )
                            TextButton(onClick = { variables.retry() }) {
                                Text(stringResource(R.string.variables_retry))
                            }
                        }
                    }
                    else -> item {
                ...
                // Paging 3.4 API: stream via items(count = ...) with stable
                // key/contentType factories (items(LazyPagingItems) was removed).
                items(
                    count = variables.itemCount,
                    key = variables.itemKey { it.id },
                    contentType = variables.itemContentType { "variable" }
                ) { index ->
                    val variable = variables[index] ?: return@items
                    NexaFlowCard {
                        Row(
                            modifier = Modifier.fillMaxWidth(),
                            verticalAlignment = Alignment.CenterVertically,
                ...
                when (val append = variables.loadState.append) {
                    is LoadState.Loading -> item {
                        Box(
